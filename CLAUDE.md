...
> **定位（已於開發中期確立）**：為與第 5 組「海象資料視覺化」做出結構性差異，本系統重新定位為
> **「海勤人力資源與作業安全決策系統」**——海象僅為輸入訊號，真正的輸出是人員疲勞、工時公平、
> 船艦可用性與**自動產生的明日值勤班表**（`build_schedule()`）。這些都需要人員/勤務/船艦資料，
> 是純海象視覺化系統做不出來的決策維度。一句話：別人把海象畫成圖，我們把海象變成「明天誰上哪艘船」。
...
| Gradio 互動介面 | ✅ | 選擇性 |
| scikit-learn（RandomForest 海況預測） | ✅ | 選擇性（差異化新增） |
| Folium 互動海域地圖 | ✅ | 選擇性（差異化新增） |
| 中央氣象署開放資料（CWA 浮標海象） | ✅ | 選擇性（差異化新增） |
| Keras 預訓練模型 | ❌ | 選擇性（以 scikit-learn 取代，控制範圍） |
