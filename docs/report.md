...
左側設定篩選條件，右側以分頁呈現所有分析結果：

**篩選條件（左欄）**
- **日期區間**：輸入開始／結束日期，限縮分析時間範圍
- **值勤海域**：勾選港口、近海、外海（可多選，全選等同無篩選）
- **船艦篩選**：從 DB 動態載入船艦清單，支援全選 / 清空快捷按鈕
...
- `analysis.py` 模組化為純函式，`app.py` 直接 import `generate_charts` 與 `compute_recommendations`，避免程式碼重複
- 篩選在 SQL 層執行（非 Python 層過濾），降低大資料量下的記憶體消耗
- 篩選結果存為 `filtered_*.png` 與 `filtered_recommendations.json`，不覆蓋全覽結果，PHP 儀表板持續可用
- `_rec_to_html()` 將決策建議 dict 轉為 HTML 字串，以 `gr.HTML` 呈現，無需額外 Gradio 元件
- Gradio 6 的 `allowed_paths` 參數確保 OUTPUT_DIR 外的圖檔可被正常提供，規避沙盒限制
...
- `monthly_trend.png`：值勤量冬季（11–12月）略低，春季（3–4月）回升，符合海事作業季節性；月度變異可識別人力尖峰
...
- `zone_bar.png`：近海 ≈ 40%、港口 ≈ 35%、外海 ≈ 25%，反映近岸巡邏為主要任務型態
- `zone_sea_stacked.png`：外海大浪比例達 20%（港口僅 1%），顯示海域分配對人員暴露風險有顯著影響
- `hours_boxplot.png`：大浪天中位值勤時數較平靜天低 1.5 小時，單因子 ANOVA（p < 0.05）確認差異顯著
- `hours_heatmap.png`：外海 × 大浪的平均工時最低，交互效應顯示海域與海況共同決定實際出勤時長
- `sea_obs_comparison.png`：模擬海況分布與中央氣象署觀測值之比對，驗證模擬資料的業務合理性

**資源調度（3 張）**

- `vessel_count.png`：八艘船艦值勤次數標準差低，輪班制度執行均勻，無過度集中現象
